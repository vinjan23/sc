...
$app_daemon tx distribution withdraw-rewards $val_address --from=$key_name --commission --chain-id=$chain_id  --gas-prices=$gas_prices$denom --yes
...
$app_daemon tx staking delegate $val_address $AMOUNT_STRING --from $key_name --chain-id $chain_id --gas-prices=$gas_prices$denom --yes
